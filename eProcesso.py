...
class Application(tk.Frame):
    ...
    def __init__(self, master=None):
        ...
        # widgets
        # formata texto
        ttk.Label(self, text="Formata texto para nota", style="Title.TLabel").grid(
            row=0, column=0, columnspan=6
        )
        ttk.Label(self, text="Estilo:", style="BG.TLabel").grid(
            row=1, column=0, sticky="w"
        )
        self.check_negrito = ttk.Checkbutton(
            self, text="Negrito", variable=self.valor_negrito, style="BG.TCheckbutton"
        )
        self.check_negrito.grid(row=1, column=1)
        self.valor_negrito.set("1")
        self.check_italico = ttk.Checkbutton(
            self, text="Itálico", variable=self.valor_italico, style="BG.TCheckbutton"
        )
        self.check_italico.grid(row=1, column=2)
        self.check_sublinhado = ttk.Checkbutton(
            self,
            text="Sublinhado",
            variable=self.valor_sublinhado,
            style="BG.TCheckbutton",
        )
        self.check_sublinhado.grid(row=1, column=3)
        ttk.Label(self, text="Cor:", style="BG.TLabel").grid(
            row=2, column=1, sticky="e"
        )
        self.combo_color = ttk.Combobox(
            self,
            values=["Normal", "Azul", "Verde", "Vermelho"],
        ...
        self.bt_gera_nota = tk.Button(
            self,
            style_button,
            text="Gera nota formatada",
            command=self.formata_texto_nota,
        )
        self.bt_gera_nota.grid(row=4, column=0, columnspan=6)
        tt.ToolTip(
            self.bt_gera_nota,
            "Gera nota com o texto acima formatado conforme as seleções de estilo e cor",
        )
        ttk.Separator(self, orient=tk.HORIZONTAL).grid(
            row=5, columnspan=6, padx=10, pady=5, sticky=tk.EW
        )

        # Inclui url
        ttk.Label(self, text="Inclui link (url) em nota", style="Title.TLabel").grid(
            row=6, column=0, columnspan=6
        )
        ttk.Label(self, text="Link:", style="BG.TLabel").grid(
        ...
        tt.ToolTip(
            self.bt_gera_link, "Gera nota com link para a url indicada no campo acima"
        )
        ttk.Separator(self, orient=tk.HORIZONTAL).grid(
            row=10, columnspan=6, padx=10, pady=5, sticky=tk.EW
        )

        # Inclui link para processo
        self.label_titulo_3 = ttk.Label(
            self, text="Inclui link para outro processo em nota", style="Title.TLabel"
        ).grid(row=11, columnspan=6)
        ttk.Label(self, text="Processo:", style="BG.TLabel").grid(
        ...
        self.bt_gera_link_proc = tk.Button(
            self,
            style_button,
            text="Gera link para outro processo",
            command=self.link_processo,
        )
        self.bt_gera_link_proc.grid(row=14, column=0, columnspan=6)
        tt.ToolTip(
            self.bt_gera_link_proc,
            "Gera nota com link para o processo indicado no campo acima",
        )
        ttk.Separator(self, orient=tk.HORIZONTAL).grid(
            row=15, columnspan=6, padx=10, pady=5, sticky=tk.EW
        )

        # Transpõe processos
        ttk.Label(
            self,
            text="Transpõe relação de processos copiados na memória",
            style="Title.TLabel",
        ).grid(row=16, columnspan=6)
        self.bt_transp_procs = tk.Button(
            self,
            style_button,
            text="Gera relação transposta",
            command=self.transpoe_clipboard,
        )
        self.bt_transp_procs.grid(row=17, column=0, columnspan=6)
        tt.ToolTip(
            self.bt_transp_procs,
            f"Transpõe a relação de processos copiados na memória para ser colada"
            f" no e-Processo",
        )
        ttk.Separator(self, orient=tk.HORIZONTAL).grid(
            row=18, columnspan=6, padx=10, pady=5, sticky=tk.EW
        )

        # Abre funcionalidades
        ttk.Label(self, text="Abre funções / processos", style="Title.TLabel").grid(
            row=19, columnspan=6
        )
        self.bt_abre_cx_trab = tk.Button(
            self,
            style_button,
            text="Abre Caixa de Trabalho",
            command=self.abre_caixa_trabalho,
        )
        self.bt_abre_cx_trab.grid(row=20, column=0, columnspan=6)
        tt.ToolTip(
            self.bt_abre_cx_trab, "Abre a caixa de trabalho de equipe no e-Processo"
        )

        self.bt_abre_ger = tk.Button(
            self,
            style_button,
            text="Abre Gerencial de Estoque",
            command=self.abre_gerencial_estoque,
        )
        self.bt_abre_ger.grid(row=21, column=0, columnspan=6)
        tt.ToolTip(
            self.bt_abre_ger, "Abre o gerencial de estoque de processos do e-Processo"
        )

        self.bt_abre_consulta = tk.Button(
        ...
        self.bt_abre_procs = tk.Button(
            self,
            style_button,
            text="Abre processos da área de transferência (clipboard)",
            command=self.abre_processos,
        )
        self.bt_abre_procs.grid(row=23, column=0, columnspan=6)
        tt.ToolTip(
            self.bt_abre_procs, "Abre os processos os copiados na memória no e-Processo"
        )
        ttk.Separator(self, orient=tk.HORIZONTAL).grid(
            row=24, columnspan=6, padx=10, pady=5, sticky=tk.EW
        )
    ...
    def abre_caixa_trabalho(self, event=None):
        webbrowser.open(
            "https://eprocesso.suiterfb.receita.fazenda/ControleAcessarCaixaTrabalho.asp?psAcao=apresentarPagina&psLimpaEquipe=1"
        )

    def abre_gerencial_estoque(self, event=None):
        webbrowser.open(
            "https://eprocesso.suiterfb.receita.fazenda/relatorios/ControleManterVisao.asp?psAcao=exibir"
        )

    def abre_consulta(self, event=None):
        webbrowser.open(
            "https://eprocesso.suiterfb.receita.fazenda/eprocesso/index.html#/consultaProcesso"
        )
